...
def main_menu(user_id):
    ...
    kb.add_line()
    kb.add_button("📖 Справка", color=VkKeyboardColor.SECONDARY)
    return kb.get_keyboard()
...
Общие:
/start — меню
/help — подробная справка по роли
...
def get_role_help(user_id):
    role = get_role(user_id)

    if role == "operator":
        return """📖 СПРАВКА — СТАНОЧНИК
...
🔹 СОЗДАТЬ ЗАЯВКУ
📦 Новая заявка → категория.

🪵 Пиломатериалы:
1. Толщина (10 / 16 / 22мм)
2. Декор (Дуб, Орех, Тефия…)
3. Количество — цифрой
4. План — кнопкой
5. ✅ Отправить
➜ Списывается сразу.
...
📏 Кромка:
1. Выберите план
2. «Кромочный клей нужен?» — Да / Нет
3. Если Да → количество КАНИСТР
4. ✅ Отправить

🎞 Плёнка ПВХ и клей МДФ:
1. Материал (Красный, Белый, Клей МДФ)
2. Количество — кнопками 1–9
3. Если плёнка → «Клей МДФ нужен?»
4. ✅ Отправить

🔹 КОРЗИНА
• ➕ Добавить — ещё позицию (можно с другим планом)
• 🗑 Очистить — сбросить
• ✅ Отправить заявку — завершить

🔹 ЧТО ДАЛЬШЕ
• Пиломатериалы — списаны сразу
• Кромка/плёнка — ждут кладовщика
• Когда выдадут — придёт «✅ Заявка выполнена»
...
📄 Мои заявки — история"""
    ...
    if role == "driver":
        return """📖 СПРАВКА — ВОДИТЕЛЬ ПОГРУЗЧИКА
...
🔹 ЗАЯВКИ (что везти)
Кнопка 📥 Заявки — список активных заявок.

Уведомления:
• При заявке на пиломатериалы:
  «📦 Заявка №X — списано автоматически»
• При выдаче кромки/плёнки:
  «🚚 Заказ №X собран, можно увозить в цех»
  с планом и составом

🔹 ПРИХОД
➕ Приход:
1. Категория → материал
2. Введите количество
3. Остаток пополняется

📦 Массовый приход:
1. Категория → материал → количество
2. ➕ Ещё — для остальных позиций
3. ✅ Завершить

🔹 СВОЯ ЗАЯВКА
📦 Новая заявка — как у станочника.
...
Кнопки:
• ✅ Выдал — уведомление автору и водителям
• ❌ Не выдал — уведомление автору
...
3. Название (или «-»)
4. Остаток
5. Единица (шт, м, лист, кг, канистра, рулон)

✏️ Редактировать — изменить поля.
🗑 Удалить — удалить материал.
🧹 Очистить — обнулить ВСЕ остатки.

🔹 ПРИХОД
➕ Приход — по одной позиции.
📦 Массовый приход — сразу несколько.

🔹 ИНВЕНТАРИЗАЦИЯ
📋 Инвентаризация:
1. Категория или 📦 Все
2. По очереди вводите фактические остатки
3. ⏭ Пропустить / ⏹ Завершить
4. В конце — итог с разницей

🔹 СВОДКА И ЖУРНАЛ
📊 Сводка — состояние склада
📋 Журнал — последние 20 действий

🔹 ПОЛЕЗНОЕ
/whoami — своя роль"""

    if role == "admin":
        return """📖 СПРАВКА — АДМИН

Всё то же, что у кладовщика, ПЛЮС:

🔹 👥 ПОЛЬЗОВАТЕЛИ
Список всех с ролями и ID.

🔹 КОМАНДЫ
/users — список пользователей
/setrole <ID> <роль> — сменить роль
   роли: operator / driver / warehouse / admin
/resetstock — обнулить все остатки
/whoami — своя роль

🔹 КАК ВЫДАТЬ РОЛЬ
1. Сотрудник пишет боту /start
2. Узнайте его ID (vk.com/idXXXXXX)
3. Отправьте:
   /setrole 123456789 driver
4. Пусть нажмёт /start

🔹 ВЫДАЧА ЗАЯВОК
У админа НЕТ кнопок «Выдал» / «Не выдал».
Подтверждает только кладовщик.

🔹 ОСТАЛЬНОЕ
Как у кладовщика: номенклатура, инвентаризация,
сводка, журнал, приходы."""
...
def handle_message(vk, user_id, text):
    if text == "/start" or text == "Начать" or "В меню" in text:
        ...
        return
    if text == "/help" or text == "📖 Справка":
        help_text = get_role_help(user_id)
        if len(help_text) > 3800:
            cut = help_text.rfind("\n", 0, 3800)
            if cut < 1000: cut = 3800
            send(vk, user_id, help_text[:cut], None)
            send(vk, user_id, help_text[cut:].lstrip(), back_kb())
        else:
